...
tags:
  ...
  - 云文档小组件
  - GitHub Actions
description: "记录飞博虾文档小组件从开放平台、权限、opdev 上传到 Actions 发布的完整链路，以及一堆差点把人劝退的坑。"
---
...
![飞博虾整体架构示意](assets/feishu-docs-addon/arch.png)
...
| 名字 | 它是什么 | 在飞博虾里叫啥 |
|------|----------|----------------|
| **飞书应用（App）** | 开放平台上的主体，有 App ID / Secret | 我们后台叫「尝试虾」，ID：`cli_aaac9ff9eae4dbfc` |
| **云文档小组件（Docs Addon / Block）** | 挂在应用下的一种能力，能插入到文档侧栏 | 文档里搜到的名字叫「飞博虾」 |
| **机器人 / OpenAPI 身份** | 用 `tenant_access_token` 调接口 | Actions 里拉正文、写多维表格用的就是它 |
...
飞书云文档（人在写）
   └─ 侧栏插入「飞博虾」小组件
         ...
         │  repository_dispatch
         ▼
GitHub Actions「飞博虾·文档发布」
         ├─ 用 FEISHU_APP_ID/SECRET 换 tenant token
         ├─ 拉飞书正文 → 写 docs/{nav}/{slug}.md
         ├─ 更新多维表格台账（可选，失败不该拖死正文）
         └─ git commit + push → Pages 部署
```
...
| 层级 | 路径 | 职责 |
|------|------|------|
| 小组件前端 | `feiboxia/docs-addon/` | React 面板、触发 dispatch、轮询 job |
| 发布脚本 | `feiboxia/ci-doc-publish.mjs` | 拉正文、写 Markdown、写台账、写 job 状态 |
| 工作流 | `.github/workflows/feiboxia-doc-publish.yml` | checkout → 跑脚本 → commit docs |
| 任务状态 | `feiboxia/queue/jobs/{job_id}.json` | 给面板看进度（pending / running / success / failure） |
| 绑定表 | `sync/feishu-map.json` | docToken ↔ 博文路径 |
| 台账 | 飞书多维表格 | 「待发 / 已发」稿件箱 |
...
在 [飞书开放平台](https://open.feishu.cn/app) 创建企业自建应用（个人版也能玩一截），记下：
...
| 用途 | 典型 scope |
|------|------------|
| 读云文档正文 | `docx:document:readonly`、`docs:document.content:read` |
| 下载文档图片 | `drive:file:download`、`docs:document.media:download` |
| 写多维表格台账 | `bitable:app` 或 `base:record:create` / `base:record:update` |
| （可选）发通知 | `im:message` |

**开通权限 ≠ 立刻可用。** 你还要：

1. 在「版本管理与发布」里 **创建版本并发布**  
2. 确认 **可用范围** 包含你自己（文档所有者）  
3. 否则「添加文档应用」弹窗里 **搜不到你的应用**
...
![已添加「尝试虾」为文档应用（可编辑）](assets/feishu-docs-addon/real-add-doc-app.png)
...
npm i
npm start          # 本地调试（按控制台安全域名配置）
npm run build
npm run upload     # build + opdev upload ./dist -v x.y.z
```

上传依赖 [opdev](https://open.feishu.cn/) CLI：

```powershell
opdev login        # 选 Feishu，浏览器授权
opdev whoami       # 确认不是 Not login
```
...
| 现象 | 真正原因 | 处理 |
|------|----------|------|
| `Not login` / `get lark session failed` | opdev 登录过期 | 重新 `opdev login`，与版本号无关 |
| `project.config.json is not found` | 自定义 webpack 没接官方插件 | 用 `docsAddonWebpackPlugin` 生成 |
| `Not allowed to upload` | dist 空包 / 配置不全 | 先 `build` 再 upload，别手传空目录 |
| 改了代码文档里还是旧面板 | 后台版本没发布 / 文档缓存 | 开放平台创建新版本并设为可用，刷新文档 |

上传成功后控制台会给你小组件页链接；在后台填名称「飞博虾」、图标，**再发布一版**，文档 `/` 搜索才能稳定搜到。

面板大致长这样（发送中会显示进度；重开面板会尽量恢复标题 / 栏目 / 标签）：

![飞博虾小组件：发送排队中的真实界面](assets/feishu-docs-addon/real-widget-queue.png)
...
| Secret | 用途 |
|--------|------|
| `FEISHU_APP_ID` | 与开放平台一致 |
| `FEISHU_APP_SECRET` | 同上 |
| （可选）`FEISHU_NOTIFY_OPEN_ID` | 晚间重试失败时私聊通知 |

![GitHub Actions Secrets：FEISHU_APP_ID / FEISHU_APP_SECRET](assets/feishu-docs-addon/real-github-secrets.png)
...
1. checkout  
2. 注册 job pending（写队列 JSON 并 push）  
3. 配置 lark-cli / 注入 `FEISHU_*`  
4. `node feiboxia/ci-doc-publish.mjs`  
5. 提交 job 状态；若成功再提交 `docs/`  
6. 若发布逻辑失败 → `Fail if publish failed` 故意标红  

所以你会看到一个很迷惑的画面：`Run feiboxia doc publish` 显示绿勾（开了 `continue-on-error`），后面 `Fail if publish failed` 红叉。**请点开第 7 步日志看真实原因**，不要只看红叉那一步。

![Actions：Fail if publish failed（请点开上一格日志看真因）](assets/feishu-docs-addon/real-actions-fail.png)
...
1. 读当前文档 token / 标题  
2. 合并本地草稿（`localStorage`）与 Interaction，避免重开面板被飞书标题冲掉  
3. `repository_dispatch`，带上 `job_id`、title、nav、slug、tags、doc_url…  
4. 轮询：仓库里的 job JSON + Actions runs API  
5. 成功则刷「已发布」；失败展示 message（现在会尽量带上飞书 API 原文）
...
|------|----------|------|
| 发送 | 仓库尚无对应博文 | 首次发布 |
| 重新发送 | 已有博文且已绑定本文档 | 覆盖更新 |
| 拉取 | 线上 URL 存在但未绑定本文档 | 把线上文绑到当前飞书文档 |
| 撤销 | 填了线上 URL 且文件存在 | 博客标 draft / 删绑定 |
...
1. 飞书写正文  
2. `/` 插入「飞博虾」  
3. 选栏目（如 `tech`）、填 slug / 标签、勾选 GitHub 博客  
4. 点「发送」或「重新发送」  
5. 看进度条；失败就点「Actions」看日志  
...
```powershell
cd D:\my-blog
# 用户身份拉一篇并写入 docs/（可加 --push）
npm run from-feishu -- "https://my.feishu.cn/docx/TOKEN" --push
...
2. 推错分支（`master` vs `main`）  
3. PAT 缺 `workflow`，根本没跑起来  
4. 实际已经 **failure**，面板还显示排队  
...
- Secrets 是否是**这对** App ID/Secret（别拷错应用）  
- 授权的是不是**当前这篇**文档（token 要对）  
- 个人版 / 权限延迟：改完再发一次  

本机验证：

```powershell
lark-cli docs +fetch --as bot --doc "https://my.feishu.cn/docx/TOKEN" --doc-format markdown --json
```
...
2. 开文档 / 云空间 / 多维表格相关 scope，**发一版并设可用范围**  
3. 配置 GitHub Secrets：`FEISHU_APP_ID` / `FEISHU_APP_SECRET`  
4. `opdev login` → `npm run upload` → 后台发布小组件版本  
5. 文档里插入「飞博虾」，设置里填仓库 + PAT（repo+workflow）  
6. 对该文档「添加文档应用」→ 应用名（如尝试虾）→ 可阅读  
7. 台账 Base 同样添加应用 → 可编辑  
8. 点发送；失败就看 Actions 第 7 步日志，不要只看最后红叉  
9. 仍不通时用本机 `from-feishu --push` 兜底，再回头修权限  
...
| 尝试虾应用 | <https://open.feishu.cn/app/cli_aaac9ff9eae4dbfc> |
| 小组件后台 | <https://open.feishu.cn/app/cli_aaac9ff9eae4dbfc/blocks/> |
| 给应用加文档权限（官方） | <https://open.feishu.cn/document/faq/trouble-shooting/how-to-add-permissions-to-app> |
